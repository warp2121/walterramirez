...
const SkillsSection = () => {
  ...
  const skills = [
    ...
    { icon: Monitor, title: t("skills.cad"), tags: [{ label: "AutoCAD Expert" }, { label: "SolidWorks" }, { label: "Wix" }, { label: "UX Design" }] },
    { icon: Wrench, title: t("skills.industrial"), tags: [{ label: "U2 Software ERP" }, { label: "ISO 9000" }, { label: "CNC Software" }, { label: "Vectric" }] },
    { icon: Globe, title: t("skills.trade"), tags: [{ label: "MFG.COM" }, { label: "Alibaba" }, { label: "Procurement" }, { label: "Logistics" }] },
    {
      icon: BookOpen, title: t("skills.management"),
      tags: [{ label: t("skills.tag.univ") }, { label: t("skills.tag.innov") }, { label: t("skills.tag.eval") }, { label: t("skills.tag.modern") }],
    },
    { icon: LayoutGrid, title: t("skills.software"), tags: [{ label: "MS Office" }, { label: "CNC Center" }, { label: "GPT for Coding" }] },
  ];
  ...
  return (
    <section id="habilidades" className="py-24 px-8 relative z-[1]" style={{ background: "linear-gradient(180deg, hsl(var(--background)) 0%, hsl(var(--cyber-cyan) / 0.02) 50%, hsl(var(--background)) 100%)" }}>
      <div className="max-w-6xl mx-auto">
        <div className="mb-12">
          <div className="section-number mb-2">{t("skills.section")}</div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {skill.tags.map((tag) => (
                  <span
                    key={tag.label}
                    className={`px-3 py-1 rounded-md text-xs border transition-all cursor-default ${(tag as { featured?: boolean }).featured ? "font-semibold" : ""}`}
                    style={{
                      background: (tag as { featured?: boolean }).featured ? "hsl(var(--cyber-cyan))" : "hsl(var(--cyber-cyan) / 0.1)",
                      color: (tag as { featured?: boolean }).featured ? "hsl(var(--background))" : "hsl(var(--cyber-cyan))",
                      borderColor: (tag as { featured?: boolean }).featured ? "hsl(var(--cyber-cyan))" : "hsl(var(--cyber-cyan) / 0.4)",
                      textShadow: (tag as { featured?: boolean }).featured ? "none" : "0 0 8px hsl(var(--cyber-cyan) / 0.5)",
                      boxShadow: (tag as { featured?: boolean }).featured ? "0 0 20px hsl(var(--cyber-cyan) / 0.5)" : "none",
                    }}
                  >
                    {tag.label}
                  </span>
